...
#endif
#if defined(RP2350)
// RP2350: holding comm_chan_lock() across mavlink_msg_*_send_struct() causes a nested lock: send_struct calls comm_send_lock() which tries to take the same chan_lock again.
// Even with CH_CFG_USE_MUTEXES_RECURSIVE=TRUE the chMtxTryLock() fast path used inside comm_send_lock checks can interact badly with the blocking take already held here.
...
// If a client retries OpenFileRO on the same session, recover by forcing the stale open file closed and reopening cleanly.
            close();    // error code ignored
            fd = -1;
        }
#else
