...
- A determined hacker could still use a debugger or dfu to download the firmware image an retrieve the encryption key. This can be   prevented using STM32 readout protection which can be enabled by addin this to the hwdef:
```
  define HAL_FLASH_READOUT_PROTECTION 1
```
Note that this is a one-way operation. See the comments in flash.c as to how this can be undone.