...
void lua_scripts::encrypt_all_scripts_in_dir(const char *dirname)
{
    ...
    for (struct dirent *de=AP::FS().readdir(d); de; de=AP::FS().readdir(d)) {
        ...
        // Allocate buffer
        char *buffer = (char *)malloc(filesize + 16);
        if (buffer == nullptr) {
            AP::FS().close(fd);
            return;
        }
...
void lua_scripts::create_nonce(uint8_t nonce[24], const char* scriptname)
{
    ...
    // serial id + script name should be unique, could also store the extra 12 bytes in the file
    snprintf((char*)nonce + 12, 12, "%s", scriptname);
}
