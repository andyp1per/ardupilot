...
bool AP_FlashIface_JEDEC::init()
{
    ...
    if (!detect_device()) {
        hal.console->printf("Failed to detect flash device: %s\n", supported_devices[_dev_list_idx].name);
        return false;
    }
    ...
    // i.e. 4-4-4
    if (!configure_device()) {
        hal.console->printf("Failed to config flash device: %s\n", supported_devices[_dev_list_idx].name);
        return false;
    }

    hal.console->printf("Detected Flash Device: %s\n", supported_devices[_dev_list_idx].name);
    return true;
}
//////////////////////////////////////////////////////
////////////////// Internal Methods //////////////////
 ...
 *
 */
bool AP_FlashIface_JEDEC::verify_erase(uint32_t sector)
{
 ...
 * @param[out] delay_ms     Time to wait until next is_device_busy call
 * @param[out] timeout_ms   Time after which the erase should be timedout,
 ...
 *
 */
bool AP_FlashIface_JEDEC::start_program_page(uint32_t page, uint8_t* data,
 ...
 * @param[out] delay_ms     Time to wait until program typically finishes
 * @param[out] timeout_ms   Time by which current program should have timedout.
 * @return                  The operation status.
 * @retval false            if the operation failed.
 * @retval true             if the operation succeeded.
 *
 */
bool AP_FlashIface_JEDEC::start_program_offset(uint32_t offset, uint8_t* data, uint32_t size, uint32_t &programming,
        uint32_t &delay_us, uint32_t &timeout_us)
{
