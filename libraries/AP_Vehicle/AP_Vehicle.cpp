...
void AP_Vehicle::check_motor_noise()
{
    ...
    for (uint8_t i = 0; i<numf; i++) {
        if (is_zero(esc_data[i]) || !(gyro_fft.get_post_filter_mask() & 1U<<i)) {
            continue;
        }
        ...
        if (energy > 40.0f && AP_HAL::millis() - last_motor_noise_ms > 5000) {
            gcs().send_text(MAV_SEVERITY_WARNING, "Significant motor noise %.1fdB on motor %u at %.1fHz", energy, i+1, esc_data[i]);
            output_error = true;
        }
    }
