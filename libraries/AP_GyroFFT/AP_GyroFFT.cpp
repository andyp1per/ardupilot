...
void AP_GyroFFT::calculate_noise(uint16_t max_bin)
{
    ...
    if (!_thread_state._noise_needs_calibration && !isnan(_state->_freq_bins[max_bin]) && snr > _config._snr_threshold_db) {
        ...
        weighted_center_freq_hz = MAX(_state->_max_bin_freq, (float)_config._fft_min_hz);
        weighted_center_freq_hz = MIN(weighted_center_freq_hz, (float)_config._fft_max_hz);
        ...
        // determine the bandwidth of the peak
         _thread_state._center_bandwidth_hz[_update_axis] = _center_bandwidth_filter[_update_axis].apply(calculate_noise_bandwidth_hz(max_bin));
        _missed_cycles = 0;
    }
   ...
   if (_output_count % 400 == 0)
   {
        ...
        gcs().send_text(MAV_SEVERITY_WARNING, "FFT: bw:%.1f, e:%.1f, r:%.1f, snr:%.1f",
                        _thread_state._center_bandwidth_hz[_update_axis], _state->_freq_bins[max_bin], _ref_energy[_update_axis][max_bin], snr);
...
float AP_GyroFFT::self_test(float frequency, GyroWindow test_window)
{
    ...
    uint16_t max_bin = hal.dsp->fft_analyse(_state, _config._fft_start_bin, _config._fft_end_bin);
