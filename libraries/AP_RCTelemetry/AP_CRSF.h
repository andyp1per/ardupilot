...
public:
    ...
    // do not allow copies
    AP_CRSF(const AP_CRSF &other) = delete;
    AP_CRSF &operator=(const AP_CRSF &) = delete;

    // get singleton instance
    static AP_CRSF *get_singleton() {
        return _singleton;
    }
    ...
    struct sbuf_s;
    struct clivalue_s;
    void crsfProtocolPackOutOfRangeCli(StreamBuffer* dst);
    void crsfProtocolPackFolderCli(StreamBuffer*dst, const struct clivalue_s *value, crsfProtocolParamPackString_e paramPackString);
    void crsfProtocolPackU8Cli(StreamBuffer*dst, const struct clivalue_s *value, crsfProtocolParamPackString_e paramPackString);
    void crsfProtocolUnpackU8Cli(const struct clivalue_s *value, const uint8_t *payload);
    void crsfProtocolPackU16Cli(StreamBuffer*dst, const struct clivalue_s *value, crsfProtocolParamPackString_e paramPackString);
    void crsfProtocolUnpackU16Cli(const struct clivalue_s *value, const uint8_t *payload);

    void crsfParameterWriteEntryHeader(StreamBuffer*dst, uint8_t destinationAddress);
    void crsfProtocolParameterRead(StreamBuffer*dst, const uint8_t *payload, uint8_t originAddress, crsfProtocolParamPackString_e paramPackString);
    void crsfProtocolParameterWrite(const uint8_t *payload);
    bool crsfInterpretExtendedFrame(StreamBuffer*dst, uint8_t *frame);
