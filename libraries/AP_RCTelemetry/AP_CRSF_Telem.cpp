...
void AP_CRSF_Telem::setup_wfq_scheduler(void)
{
    ...
    add_scheduler_entry(50, 100);   // parameters       10Hz
...
void AP_CRSF_Telem::process_frame(AP_RCProtocol_CRSF::FrameType frame_type, void* data) {
    switch (frame_type) {
...
void AP_CRSF_Telem::process_vtx_frame(VTXFrame* vtx) {
    ...
    if (vtx->is_in_pitmode) {
        apvtx->set_options(AP_VideoTX::VTX_PITMODE);
    }
}
...
void AP_CRSF_Telem::process_vtx_telem_frame(VTXTelemetryFrame* vtx) {
    ...
    // 14dBm (25mW), 20dBm (100mW), 26dBm (400mW), 29dBm (800mW)
    uint16_t power = uint16_t(roundf(powf(10, vtx->power / 10.0f)));
    apvtx->set_power_mw(power);
...
void AP_CRSF_Telem::update_params()
{
    ...
    if (vtx->have_params_changed()) {
        ...
        const bool update_frequency = vtx->update_band() || vtx->update_channel();
        ...
        uint8_t len = 5;
        if (update_frequency && _freq_update) {
            ...
            _freq_update = false;
            _telem_pending = true;
            // force the current band and channel to be the same as configired, if the set does not work, they will get unset
            // when we receive a VTX frame and the process will continue. This prevents flooding with update packets.
            vtx->set_channel(vtx->get_configured_channel());
            vtx->set_band( vtx->get_configured_band());
            len++;
        } else if (update_frequency) {
            ...
            _freq_update = true;
        } else if (vtx->update_power() && _dbm_update) {
            _telem.ext.command.payload[0] = AP_RCProtocol_CRSF::CRSF_COMMAND_VTX_POWER_DBM;
            uint8_t power = uint8_t(roundf(10.0f * log10f(vtx->get_configured_power_mw())));
            _telem.ext.command.payload[1] = power;
            vtx->set_power_mw(vtx->get_configured_power_mw());
            _dbm_update = false;
            ...
            _telem.ext.command.payload[1] = vtx->get_configured_options() & AP_VideoTX::VTX_PITMODE;
            vtx->set_options(vtx->get_configured_options());
        }
        _telem_pending = true;
