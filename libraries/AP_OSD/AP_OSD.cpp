...
const AP_Param::GroupInfo AP_OSD::var_info[] = {
    ...
    AP_SUBGROUPINFO(param_screen[0], "5_", 20, AP_OSD, AP_OSD_ParamScreen),
...
void AP_OSD::update_current_screen()
{
    ...
    if (AP_Notify::flags.armed) {
        if (!was_armed && arm_scr > 0 && arm_scr <= AP_OSD_NUM_SCREENS && get_screen(arm_scr-1).enabled) {
            current_screen = arm_scr-1;
        }
        was_armed = true;
    } else if (was_armed) {
        if (disarm_scr > 0 && disarm_scr <= AP_OSD_NUM_SCREENS && get_screen(disarm_scr-1).enabled) {
    ...
    if (AP_Notify::flags.failsafe_radio || AP_Notify::flags.failsafe_battery) {
        if (!was_failsafe && failsafe_scr > 0 && failsafe_scr <= AP_OSD_NUM_SCREENS && get_screen(failsafe_scr-1).enabled) {
