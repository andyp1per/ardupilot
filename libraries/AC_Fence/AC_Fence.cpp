...
void AC_Fence::update()
{
    ...
    if (get_enabled_fences() && last_msg_count++ % 10 == 0) {
        ...
        AC_Fence::get_fence_names(get_enabled_fences(), e);
        GCS_SEND_TEXT(MAV_SEVERITY_NOTICE, "%s fences active", e.get_writeable_string());
    }
#endif
}
    ...
    switch(auto_enabled()) {
        ...
        case AC_Fence::AutoEnable::ENABLE_DISABLE_FLOOR_ONLY: {
            ...
            if (fences) {
                ...
                GCS_SEND_TEXT(MAV_SEVERITY_NOTICE, "%s auto-enabled", e.get_writeable_string());
            }
            break;
        }
        default:
    ...
    switch (auto_enabled()) {
        case AC_Fence::AutoEnable::ENABLE_ON_AUTO_TAKEOFF: {
            ...
            if (fences) {
                ...
                ExpandingString e(msg, MAVLINK_MSG_STATUSTEXT_FIELD_TEXT_LEN+1);
                AC_Fence::get_fence_names(fences, e);
                GCS_SEND_TEXT(MAV_SEVERITY_NOTICE, "%s auto-disabled", e.get_writeable_string());
            }
            break;
        }
        case AC_Fence::AutoEnable::ENABLE_DISABLE_FLOOR_ONLY:
