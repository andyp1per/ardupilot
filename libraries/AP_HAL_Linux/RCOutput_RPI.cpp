...
RCOutput_RPI::RCOutput_RPI()
#if defined(HAL_LINUX_SERIALLED_ENABLED) && HAL_LINUX_SERIALLED_ENABLED
    : _serial_led(nullptr)
#endif
{
}
...
void RCOutput_RPI::init()
{
    ...
    _serial_led = NEW_NOTHROW SerialLED_SPI("/dev/spidev0.0", 1);
    if (_serial_led != nullptr) {
        if (!_serial_led->init()) {
            delete _serial_led;
            _serial_led = nullptr;
        }
...
{
#if defined(HAL_LINUX_SERIALLED_ENABLED) && HAL_LINUX_SERIALLED_ENABLED
    if (_serial_led == nullptr || !_serial_led->initialized()) {
        return false;
    }
