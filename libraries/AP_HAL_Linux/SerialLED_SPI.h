...
public:
    /*
     * Constructor for the serial LED driver.
     *
     * @param spi_device Path to the SPI device (e.g., "/dev/spidev0.0")
     * @param num_channels Number of LED channels to support (max 4)
     */
    SerialLED_SPI(const char *spi_device, uint8_t num_channels);
    ~SerialLED_SPI();
