...
SerialLED_SPI::~SerialLED_SPI()
{
    ...
    for (uint8_t i = 0; i < MAX_CHANNELS; i++) {
        delete[] _channels[i].led_data;
    }
}

bool SerialLED_SPI::init()
{
    _spi_buffer = NEW_NOTHROW uint8_t[SPI_BUFFER_SIZE];
...
bool SerialLED_SPI::set_num_leds(uint16_t chan, uint8_t num_leds, AP_HAL::RCOutput::output_mode mode)
{
    ...
    // Allocate or reallocate LED data array
    delete[] ch.led_data;
    ch.led_data = NEW_NOTHROW SerialLed[num_leds];
    ...
    // Initialize to off
    memset(ch.led_data, 0, sizeof(SerialLed) * num_leds);
...
{
    if (!_initialized || chan >= _num_channels || _spi_fd < 0) {
        return false;
    }
...
void SerialLED_SPI::encode_led_data(uint8_t channel)
{
    ...
    memset(_spi_buffer, 0, SPI_BUFFER_SIZE);
