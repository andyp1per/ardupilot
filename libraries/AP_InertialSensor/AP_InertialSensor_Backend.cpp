                                                            ...
                                                            uint64_t sample_us)
{
...

    {
        ...
        if (_imu._gyro_window_size > 0) {
            ...
            // we always favour the writer if the buffer is full
            // since up-to-date data is better
            if (!_imu._gyro_window[instance][0].space()) {
                !_imu._gyro_window[instance][0].advance(1);
            }
            _imu._gyro_window[instance][0].push(scaled_gyro.x);
            if (!_imu._gyro_window[instance][1].space()) {
                !_imu._gyro_window[instance][1].advance(1);
            }
            _imu._gyro_window[instance][1].push(scaled_gyro.y);
            if (!_imu._gyro_window[instance][2].space()) {
                !_imu._gyro_window[instance][2].advance(1);
            }
