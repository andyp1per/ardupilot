...
public:
    ...
    virtual void fft_start(FFTWindowState* state, const float* samples, uint16_t buffer_index) = 0;
    // perform remaining steps of an FFT analysis
    virtual uint16_t fft_analyse(FFTWindowState* state, uint16_t start_bin, uint16_t end_bin) = 0;
