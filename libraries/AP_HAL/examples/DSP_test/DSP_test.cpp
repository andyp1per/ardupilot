...
static float test_window[WINDOW_SIZE];
...
protected:
    ...
    virtual void vector_mean_float(const float* vin, uint16_t len, float* mean_value) const override {};
public:
    void run_tests();
} dsptest;
...
void setup()
{
    ...
    for(uint16_t i = 0; i < WINDOW_SIZE; i++) {
        test_window[i] = sinf(2.0f * M_PI * frequency1 * i / SAMPLE_RATE) * ToRad(20) * 2000;
        test_window[i] += sinf(2.0f * M_PI * frequency2 * i / SAMPLE_RATE) * ToRad(10) * 2000;
        test_window[i] += sinf(2.0f * M_PI * frequency3 * i / SAMPLE_RATE) * ToRad(10) * 2000;
    }
...
void do_fft(const float* data)
{
    hal.dsp->fft_start(fft, data, 0, WINDOW_SIZE);
