...
AP_GPS_UBLOX::_request_next_config(void)
{
    ...
    // done with the autoconfiguration
    ...
    case STEP_F9: {
        if (_hardware_generation == UBLOX_F9 && gps._gnss_mode[state.instance] != 0) {
    ...
    if (_class == CLASS_ACK) {
        ...
        if(_msg_id == MSG_ACK_NACK) {
            ...
            case CLASS_CFG:
                ...
                case MSG_CFG_VALGET:
                    Debug("NACK 0x%x", (unsigned)_buffer.nack.msgID);
                    if (active_config.list != nullptr) {
                        /*
    ...
    if (_class == CLASS_CFG) {
        ...
        case MSG_CFG_VALGET: {
            ...
            while (cfg_len >= 5) {
                ...
                if (cfg_idx >= 0) {
                    Debug("valset(0x%lx): %u", uint32_t(id), (*cfg_data) & 0x1);
                    ...
                    Debug("valget no active config for 0x%lx", (uint32_t)id);
                }
...
AP_GPS_UBLOX::_save_cfg()
{
    ...
    _num_cfg_save_tries++;
#ifndef COLLMOT_HACKS_ENTRON_300
    GCS_SEND_TEXT(MAV_SEVERITY_INFO,
