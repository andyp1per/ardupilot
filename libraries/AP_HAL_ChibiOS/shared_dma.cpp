...
bool Shared_DMA::lock_stream(uint8_t stream_id)
{
    ...
    if (stream_id < SHARED_DMA_MAX_STREAM_ID) {
        // someone else currently has the lock, ignore that it might be us - just for stats
        cont = locks[stream_id].mutex.owner != nullptr;
...
#define STREAM_OFFSET 1
        ...
        n = hal.util->snprintf(buf, bufsize, fmt, i / STREAM_MUX + 1, i % STREAM_MUX + STREAM_OFFSET,  _contention_stats[i].transactions,
