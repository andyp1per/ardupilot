...
// RP2350 dual-core dispatch: rate_thread.cpp dispatches rate_controller_run_dt() to core1 via c1_run_sync() when FSTRATE_ENABLE=1 and RP_CORE1_START=TRUE.
// The rate thread itself still runs on core0 (thread scheduling is not affected here).
