...
#if W25Q_BUS_MODE == W25Q_BUS_MODE_WSPI1L
static const uint8_t W25Q_evconf_value[1] = {0xCF};
#elif W25Q_BUS_MODE == W25Q_BUS_MODE_WSPI2L
static const uint8_t W25Q_evconf_value[1] = {0x8F};
#else
static const uint8_t W25Q_evconf_value[1] = {0x4F};
#endif
...
void snor_device_init(SNORDriver *devp) {
  ...
  wspiSend(devp->config->busp, &W25Q_cmd_write_evconf, 1, W25Q_evconf_value);
...
#if SNOR_BUS_DRIVER == SNOR_BUS_DRIVER_WSPI
  {
    ...
    bus_cmd_send(devp->config->busp, W25Q_CMD_WRITE_V_CONF_REGISTER,
                 1, flash_conf);
  }
#endif
}
...
flash_error_t snor_device_query_erase(SNORDriver *devp, uint32_t *msec) {
  ...
  /* Read status command.*/
  bus_cmd_receive(devp->config->busp, W25Q_CMD_READ_FLAG_STATUS_REGISTER,
                  1, &sts);
  ...
  if (((sts & W25Q_FLAGS_PROGRAM_ERASE) == 0U) ||
      ((sts & W25Q_FLAGS_ERASE_SUSPEND) != 0U)) {
  ...
  /* Checking for errors.*/
  if ((sts & W25Q_FLAGS_ALL_ERRORS) != 0U) {

    /* Clearing status register.*/
    bus_cmd(devp->config->busp, W25Q_CMD_CLEAR_FLAG_STATUS_REGISTER);
...
void snor_activate_xip(SNORDriver *devp) {
  static const uint8_t flash_status_xip[1] = {
    (W25Q_READ_DUMMY_CYCLES << 4U) | 0x07U
  };
...
void snor_reset_xip(SNORDriver *devp) {
  ...
  cmd.alt   = 0xFFU;
  cmd.addr  = 0U;
  cmd.dummy = W25Q_READ_DUMMY_CYCLES - 2U;
