...
void AP_Logger_Block::periodic_10Hz(const uint32_t now)
{
    ...
    case StatusMessage::RECOVERY_COMPLETE:
        ...
        status_msg = StatusMessage::NONE;
        break;
    case StatusMessage::LOGGING_STOPPED:
...
void AP_Logger_Block::io_timer(void)
{
    ...
    if (!_initialised || AP_HAL::millis() < 2000) {
        return;
    }
    ...
    if (!CardInserted() || new_log_pending) {
        return;
    }
    ...
    if (writebuf.available() >= df_PageSize - sizeof(struct PageHeader)) {
        WITH_SEMAPHORE(sem);
        // if we have filled the card, stop writing
        if (df_Write_FilePage > df_NumPages) {
            status_msg = StatusMessage::LOGGING_STOPPED;
            return;
        }
        write_log_page();
    }
