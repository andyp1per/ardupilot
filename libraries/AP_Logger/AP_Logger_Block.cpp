...
#define CRITICAL_MESSAGE_SPACE 1024
...
bool AP_Logger_Block::_WritePrioritisedBlock(const void *pBuffer, uint16_t size, bool is_critical)
{
    ...
    if (_writing_startup_messages &&
        ...
        if (!is_critical && space < CRITICAL_MESSAGE_SPACE) {
            _dropped++;
            write_sem.give();
            return false;
        }
...
void AP_Logger_Block::start_new_log(void)
{
    ...
    // we could be logging at the moment and have been asked to start a new
    // log. There is no need to finish the old one - just point at the new
    // state variables and the io thread will take care of the rest.
    // this avoids writing to the chip outside the IO thread. We will necessarily
